...
impl Parser {
    pub fn conditional_and(&mut self) -> Result<Box<Node>, AxiomError> {
            ...
            match token {
                Token::Operator(operator_token) => {
                    match operator_token.operator_type {
                        OperatorCategory::Logical(OperatorLogicalType::And) => {
                            left = self.binary_operation(left, operator_token.location(), BinaryType::And, |_self| {_self.equality()})?;
                        }
                        _ => break
                    }
                }
